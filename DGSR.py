...
#     def __init__(self, alpha=0.25, gamma=0, reduction='mean'):
#         """
#         Initialize Focal Loss with a single alpha balancing factor.
        
#         Args:
#             alpha (float): Global weighting factor (default: 1.0)
...
#         # Compute focal loss with single alpha value
...
class DGSR(nn.Module):
    def __init__(self, user_num, item_num, input_dim, item_max_length, user_max_length, feat_drop=0.2, attn_drop=0.2,
                 ...
                 item_update='rnn', last_item=False, layer_num=3, time=True):
    ...
    def forward(self, g, user_index=None, last_item_index=None, neg_tar=None, is_training=False, return_user_profiles=False):
        feat_dict = None
        user_layer = []
        ...
        if self.layer_num > 0:
            for conv in self.layers:
                ...
                user_profile = torch.cat(user_layer, -1)
            if self.last_item:
        ...
        # score = self.mlp(unified_embedding) # this is not right, need to be fixed

        if is_training:
            if return_user_profiles:
                user_ids = g.nodes['user'].data['user_id'].cpu().numpy()
                user_profiles_dict = {user_id: embedding.cpu().detach().tolist() for user_id, embedding in zip(user_ids, user_profile)}
                unified_embeddings_dict = {user_id: embedding.cpu().detach().tolist() for user_id, embedding in zip(user_ids, unified_embedding)}
                return score, user_profiles_dict, unified_embeddings_dict
            else:
                return score
        else:
            # neg_embedding = self.item_embedding(neg_tar)
            # score_neg = torch.matmul(unified_embedding.unsqueeze(1), neg_embedding.transpose(2, 1)).squeeze(1) # original code
            # score_neg = torch.matmul(unified_embedding.unsqueeze(1), self.item_embedding.weight.transpose(1, 0)).squeeze(1) # trial code
            score_neg = score
...
class DGSRLayers(nn.Module):
    def __init__(self, in_feats, out_feats, user_max_length, item_max_length, feat_drop=0.2, attn_drop=0.2, user_long='orgat', user_short='att',
        ...
        # Initialize LSTM layers if selected
        if self.user_update_m == 'lstm':
            self.lstm_u = nn.LSTM(input_size=in_feats, hidden_size=in_feats, batch_first=True)
        if self.item_update_m == 'lstm':
            self.lstm_i = nn.LSTM(input_size=in_feats, hidden_size=in_feats, batch_first=True)

        if self.user_long in ['orgat', 'gcn', 'gru', 'lstm'] and self.user_short in ['last','att', 'att1']:
            self.agg_gate_u = nn.Linear(self.hidden_size * 2, self.hidden_size, bias=False)
        if self.item_long in ['orgat', 'gcn', 'gru', 'lstm'] and self.item_short in ['last', 'att', 'att1']:
    ...
    def user_update_function(self, user_now, user_old):
        ...
        elif self.user_update_m == 'lstm':
            user_now = user_now.unsqueeze(1)
            user_old = user_old.unsqueeze(1)
            _, (hidden, _) = self.lstm_u(torch.cat([user_now, user_old], dim=1))
    ...
    def item_update_function(self, item_now, item_old):
        ...
        elif self.item_update_m == 'lstm':
            item_now = item_now.unsqueeze(1)
            item_old = item_old.unsqueeze(1)
            _, (hidden, _) = self.lstm_i(torch.cat([item_now, item_old], dim=1))
            return hidden.squeeze(0)
        else:
    ...
    def item_reduce_func(self, nodes):
        ...
        elif self.item_long == 'lstm':
            rnn_order = torch.sort(nodes.mailbox['time'], 1)[1]
            _, (hidden_u, _) = self.lstm_i(nodes.mailbox['user_h'][torch.arange(length).unsqueeze(1), rnn_order])
            h.append(hidden_u.squeeze(0))
    ...
    def user_reduce_func(self, nodes):
        h = []
        # 先根据time排序
        order = torch.argsort(torch.argsort(nodes.mailbox['time'], 1),1)  # order of items in the original sequence
        re_order = nodes.mailbox['time'].shape[1] - order -1  # relative order of items
        ...
        elif self.user_long == 'lstm':
            rnn_order = torch.sort(nodes.mailbox['time'], 1)[1]
            _, (hidden_i, _) = self.lstm_u(nodes.mailbox['item_h'][torch.arange(length).unsqueeze(1), rnn_order])
            h.append(hidden_i.squeeze(0))
...
def collate_test(data, user_neg):
    # 生成负样本和每个序列的长度 translation: generate negative samples and the length of each sequence
    user = []
    graph = []
    label = []
    last_item = []
...
#         label.append(da[1]['target'])
#         last_item.append(da[1]['last_alis'])
#     return torch.tensor(user).long(), dgl.batch(graph), torch.tensor(label).long(), torch.tensor(last_item).long()
