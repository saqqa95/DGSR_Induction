...
def generate_user(user, data, graph, item_max_length, user_max_length, train_path, test_path, k_hop=3, val_path=None):
    ...
    if len(u_seq) < 3:
        ...
        for j, t  in enumerate(u_time[0:-1]):
            ...
            sub_u_eid = (graph.edges['by'].data['time'] < u_time[j+1]) & (graph.edges['by'].data['time'] >= start_t) # filtering edges by time  to include only those edges that fall within the specified time window
